...
void main() {
  for (final provider in [
    ...
    group(
      provider == currentWeatherFreshnessProvider
          ? 'currentWeatherFreshnessProvider'
          : 'forecastFreshnessProvider',
      () {
        ...
        test('report updates the state with the given freshness', () {
          ...
          container
              ...
              .report(
                WeatherFreshness(isFromCache: true, fetchedAt: fetchedAt),
              );
  ...
  test('the current-weather and forecast freshness providers are independent — '
    ...
    expect(
      container.read(currentWeatherFreshnessProvider)!.isFromCache,
      isTrue,
    );
