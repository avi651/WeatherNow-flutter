...
void main() {
  ...
  test(
    'resolves to the reverse-geocoded city for the device location',
    () async {
      when(
  ...
  test(
    'resolves to null when reverse geocoding fails, without throwing',
    () async {
      when(
        () => mockLocationService.getCurrentLocation(),
      ).thenAnswer((_) async => const Right(deviceLocation));
      when(
        ...
        (_) async => const Left(LocationUnavailableFailure('No match')),
      );
