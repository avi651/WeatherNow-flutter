...
void main() {
  ...
  group('GeocodingRepositoryImpl.searchCities', () {
    ...
    test('returns a RemoteDataFailure carrying the status code when the data source throws', () async {
      when(
        () => mockDataSource.searchCities(query: query),
    ...
    test('returns an empty list on success with no matches', () async {
      ...
      expect(result.isRight(), isTrue);
      result.fold((_) => fail('expected Right'), (cities) => expect(cities, isEmpty));
    });

    test('maps a real multi-result geocoding response (captured for "Pune")', () async {
      // Verbatim shape of
      ...
      when(() => mockDataSource.searchCities(query: 'Pune')).thenAnswer(
        (_) async => [
          ...
          {'name': 'pune', 'lat': 16.7541259, 'lon': 74.6513915, 'country': 'IN', 'state': 'Maharashtra'},
          {'name': 'Pune', 'lat': 1.7837578, 'lon': 127.8542945, 'country': 'ID', 'state': 'North Maluku'},
          {'name': 'Pune', 'lat': -9.36924, 'lon': 124.31618, 'country': 'TL', 'state': 'Oecussi-Ambeno'},
        ],
      );
  ...
  group('GeocodingRepositoryImpl.reverseGeocode', () {
    ...
    test('returns the mapped CitySuggestion on success', () async {
      when(
        () => mockDataSource.reverseGeocode(latitude: latitude, longitude: longitude),
      ).thenAnswer(
        (_) async => [
          {'name': 'Pune', 'country': 'IN', 'state': 'Maharashtra', 'lat': latitude, 'lon': longitude},
        ],
      );

      final result = await repository.reverseGeocode(latitude: latitude, longitude: longitude);
