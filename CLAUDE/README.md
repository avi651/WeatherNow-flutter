...
- Compile-time configuration such as `--dart-define` does not guarantee that an API key remains secret inside a distributed mobile application. Production systems should apply appropriate server-side controls, key restrictions, and monitoring.

### Hive Local Storage Security

- Hive is used for local persistence of favorites, cached weather data, forecast data, and application settings.
- Existing Hive boxes, box names, keys, and data models must be preserved when introducing security improvements.
- Sensitive Hive data should be protected using Hive encryption where implemented and verified.
- Encryption must be initialized before opening encrypted Hive boxes.
- Encryption keys must never be hardcoded, committed to source control, or printed in logs.
- The encryption status of each Hive box must be verified against the actual implementation; documentation must not claim that every box is encrypted unless this has been confirmed.

### Secure Encryption Key Management

- `flutter_secure_storage` should be used to securely store and retrieve the Hive encryption key.
- On iOS, secure storage should use the Keychain when configured and verified.
- On Android, secure storage should use Android Keystore-backed protection when configured and verified.
- The encryption key should be generated securely and reused for the corresponding encrypted Hive box.
- Missing, invalid, or corrupted encryption keys must be handled without silently deleting existing user data.
- Secure storage reduces exposure of encryption keys but does not provide absolute protection on a compromised device.

### Encryption Migration and Data Preservation

- Existing unencrypted Hive boxes must be reviewed before enabling encryption.
- Encryption changes must include a safe migration strategy where required.
- The application must not delete, reset, or overwrite existing user data as a default migration action.
- Migration compatibility, corrupted data handling, and backup considerations should be verified before release.
- Any migration limitation must be documented clearly.

### iOS Jailbreak Detection

- iOS Jailbreak detection should be documented only when the implementation is present and verified in the source code.
- Jailbreak detection is a defense-in-depth control and cannot guarantee that a device is uncompromised.
- Detection may produce false positives and can be bypassed by advanced attackers.
- The application response to a detected jailbreak must be documented according to the actual implementation, such as logging, warning, restricted behavior, or blocking.
- Unsupported-device and detection-error handling must not cause unintended application crashes.

### Android Root Detection

- Android Root detection should be documented only when the implementation is present and verified in the source code.
- Root detection is a defense-in-depth control and cannot guarantee that a device is uncompromised.
- Detection may produce false positives and can be bypassed by advanced attackers.
- The application response to a detected rooted device must be documented according to the actual implementation.
- Detection failures and unsupported environments must be handled safely without breaking normal application functionality.

### Logging and Error Handling

- Sensitive values such as API keys, encryption keys, credentials, and tokens must not be written to logs.
- Raw Dio exceptions, stack traces, parser errors, and internal implementation details must not be shown directly to users.
- Request and response logging must redact sensitive headers, query parameters, and payload fields where applicable.
- Security-related errors should be mapped to safe, user-friendly messages.

### Security Limitations

- Local encryption protects stored data at rest but does not guarantee protection against a fully compromised device.
- Jailbreak and Root detection are not foolproof security mechanisms.
- Mobile application binaries can be reverse-engineered, and compile-time configuration values may be discoverable.
- Security controls should be layered with secure key management, minimal sensitive data storage, safe logging, dependency updates, and backend-side validation.
